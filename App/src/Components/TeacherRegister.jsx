...
import { useGetDepartmentsQuery, useSignupMutation } from "../Store";
import LoadingContext from "../Context/LoadingContext";
import Toaster from "./Toaster";

const TeacherRegister = () => {
  const [signup, result] = useSignupMutation();
  const { data, isSuccess, isError, isLoading } = useGetDepartmentsQuery();
  ...
  useEffect(() => {
    ...
    if (isError || isSuccess || result.isSuccess || result.isError) {
      setProgress(100);
    }
  ...
  return (
    <div className="hero min-h-screen bg-base-200">
      ...
      <div className="flex-shrink-0 w-full max-w-lg shadow-2xl bg-base-100 px-6 py-5">
        <p className="text-3xl font-semibold text-center">Teacher Register</p>
        ...
        <form className="min-w-max prose">
          ...
          <div className="form-control py-1">
            <label className="label label-text">Teacher Id</label>
            <input
              name="student_id"
              type="text"
            ...
            />
            {teacher_id_error ? (
              <p className="text-error justify-end">{teacher_id_error}</p>
            ) : null}
          </div>
          <div className="form-control py-1">
